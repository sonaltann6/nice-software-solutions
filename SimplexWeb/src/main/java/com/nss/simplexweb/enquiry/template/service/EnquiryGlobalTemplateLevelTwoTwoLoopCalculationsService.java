...
@Service("enquiryGlobalTemplateLevelTwoTwoLoopCalculationsService")
public class EnquiryGlobalTemplateLevelTwoTwoLoopCalculationsService {
	...
	public EnquiryTemplateBean calculateBagWeight(EnquiryTemplateBean enquiryTemplateBean) {
		...
		// Adding Loop Cover
		double loopCover = 30; // in gm
		int bagDoc = 0, bagLabel = 0, bagTieForTop = 0, bagTieForBottom = 0;
		
		//1. bag tube 
		bagTube = calculateBagTube(enquiryTemplateBean.getSurfaceLength(), enquiryTemplateBean.getSurfaceWidth(), enquiryTemplateBean.getSurfaceHeight(), enquiryTemplateBean.getFabricGSMValue(), enquiryTemplateBean.getLoopHeight());
		
		//2. bag base
		bagBase = calculateBagBase(enquiryTemplateBean.getSurfaceLength(), enquiryTemplateBean.getSurfaceWidth(), enquiryTemplateBean.getFabricGSMValue());
		...
		bagWeight = (bagWeight + bagTube + loopCover + bagBase + bagInletAttachment + bagOutletAttachment + bagThread  + bagLiner + bagFillerCord + bagTieForTop + bagTieForBottom + bagDoc + bagLabel + bagPerimeterBand + bagBellyBand + bagSteevdoorStrapBand + bagBaffelGSM + bagLinerGlued + bagLoopProtector) / 1000;
	...
	private Double calculateBagBase(Double surfaceLength, Double surfaceWidth,Double fabricGSM) {
		...
		System.out.println("calculateBase="+bagBase);
		return bagBase;
	}
