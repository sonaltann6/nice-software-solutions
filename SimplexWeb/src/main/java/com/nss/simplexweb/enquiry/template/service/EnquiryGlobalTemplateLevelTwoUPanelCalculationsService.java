...
@Service("enquiryGlobalTemplateLevelTwoUPanelCalculationsService")
public class EnquiryGlobalTemplateLevelTwoUPanelCalculationsService {
	...
	public EnquiryTemplateBean calculateBagWeight(EnquiryTemplateBean enquiryTemplateBean) {
		Double bagWeight = 0.0, bagBody = 0.0, bagSide = 0.0, bagLoop = 0.0, bagInletAttachment = 0.0, bagOutletAttachment = 0.0,
		...
		//if surface type is internal
		if(enquiryTemplateBean.getProductSurfaceType().getSurfaceTypeAbbr().equalsIgnoreCase(ENQUIRY.SURFACE_TYPE_INTERNAL)) {
			
			//1. bag body and side
			bagBody = calculateInternalUPanelBody(enquiryTemplateBean.getSurfaceLength(), enquiryTemplateBean.getSurfaceHeight(), enquiryTemplateBean.getFabricGSMValue());
			
			bagSide = calculateInternalBagSide(enquiryTemplateBean.getSurfaceLength(),enquiryTemplateBean.getSurfaceWidth(), enquiryTemplateBean.getSurfaceHeight(), enquiryTemplateBean.getFabricGSMValue());
			
			//2. bag loop
			bagLoop = calculateBagCrossCornerLoop(enquiryTemplateBean.getSwl(), enquiryTemplateBean.getProductSFtype().getSfTypeValue(), enquiryTemplateBean.getLoopHeight(), enquiryTemplateBean.getSurfaceHeight(), enquiryTemplateBean.getLoopType().getLoopTypeAbr(), enquiryTemplateBean.getLoopNumber());
			
			//3. bag inlet / top filling
			...
			// 5. bag thread
						if (enquiryTemplateBean.isTopFilling()) {
							...
							} else if (enquiryTemplateBean.getTopType().getTopTypeAbbr().equalsIgnoreCase(ENQUIRY.TOP_TYPE_TOP_SPOUT) || enquiryTemplateBean.getTopType().getTopTypeAbbr().equalsIgnoreCase(ENQUIRY.TOP_TYPE_TOP_SPOUT_WITH_FLAP)) {
		...
		//if surface type is external
		else if(enquiryTemplateBean.getProductSurfaceType().getSurfaceTypeAbbr().equalsIgnoreCase(ENQUIRY.SURFACE_TYPE_EXTERNAL)) {
			
			//1. bag body and side
			bagBody = calculateExternalUPanelBody(enquiryTemplateBean.getSurfaceLength(), enquiryTemplateBean.getSurfaceHeight(), enquiryTemplateBean.getFabricGSMValue());
			
			bagSide = calculateExternalBagSide(enquiryTemplateBean.getSurfaceLength(),enquiryTemplateBean.getSurfaceWidth(), enquiryTemplateBean.getSurfaceHeight(), enquiryTemplateBean.getFabricGSMValue());
		...
		bagWeight = (bagBody + bagSide + bagLoop + bagInletAttachment + bagOutletAttachment +
				bagThread + bagLiner + bagFillerCord + bagPerimeterBand + bagBellyBand + bagSteevdoorStrapBand + bagBaffelGSM + bagLinerGlued + bagLoopProtector + bagDoc + bagLabel + bagTieForTop + bagTieForBottom) / 1000;
	...
	//1. calculate bag body
	private Double calculateInternalUPanelBody(Double surfaceLength, Double surfaceHeight, Double fabricGSM) {
		Double bagBody = 0.0;
		Double bagCutLength = calculateInternalCutLength(surfaceHeight,surfaceLength);
		surfaceLength = surfaceLength + 12;
		bagBody = (surfaceLength * bagCutLength * fabricGSM) / 10000;
		System.out.println("bag body: "+bagBody);
		return bagBody;
	}
		//1.a internal cut length
		private Double calculateInternalCutLength(Double surfaceHeight, Double surfaceLength) {
			Double bagCutLength = 0.0;
			bagCutLength = (surfaceHeight * 2) + (surfaceLength + 14);
			return bagCutLength;
		}
		
	//1. calculate bag side
	private Double calculateInternalBagSide(Double surfaceLength, Double surfaceWidth, Double surfaceHeight, Double fabricGSM) {
		double bagSide = ((surfaceLength + 12) * (surfaceHeight + 12) * fabricGSM * 2)  / 10000 ;
		return bagSide;
	}
	
	//2. calculate bag loop
	private Double calculateBagCrossCornerLoop(double SWL, int sf, double loopHeight, double surfaceHeight, String loopType, double loopNumber) {
		double loopCutLength= calculateLoopCutLength(surfaceHeight, loopHeight, SWL, sf);
		if(loopType.equalsIgnoreCase(ENQUIRY.LOOP_TYPE_CORNER_LOOP)) {
			//double loopGram = ((SWL * sf) / loopNumber) / 40;
			double loopGram = 0;
			if(sf > 5) {
				loopGram = setCornerLoopGramFromRange(SWL);
			}else {
				loopGram = setCornerLoopGramSFFiveFromRange(SWL);
			}
			System.out.println("Loop Gram : "+loopGram);
			double cornerLoop = (4 * (loopGram * loopCutLength)) / 100;
			return cornerLoop;
		}else if(loopType.equalsIgnoreCase(ENQUIRY.LOOP_TYPE_CROSS_CORNER_LOOP)) {
			double crossLoopGram = 0;
			if(sf > 5) {
				crossLoopGram=setCrossLoopGramFromRange(SWL);
			}else {
				crossLoopGram=setCrossLoopGramSFFiveFromRange(SWL);
			}
		...
		//2.a loop cut length
		private Double calculateLoopCutLength(Double surfaceHeight, Double loopHeight, Double SWL, int sf) {
			double loopCutLength=0,loopShortLen=0,loopLongLen=0;
			if(sf > 5) {
				loopShortLen = setLoopShortLengthFromRange(SWL) ;
				loopLongLen = setLoopLongLengthFromRange(SWL, surfaceHeight);
			}else {
				loopShortLen = setLoopShortLengthSFFiveFromRange(SWL) ;
				loopLongLen = setLoopLongLengthSfFiveFromRange(SWL, surfaceHeight);
			}
			loopCutLength = (2 * loopHeight) + loopShortLen + loopLongLen;
			return loopCutLength;
		}
		
		//2.a.1
		private double setLoopShortLengthFromRange(double SWL){
			if (SWL <= 499) {
	        	return 30 ;
	        } else if (SWL >= 500 && SWL <= 600) {
	        	return 40;
	        } else if (SWL >= 601 && SWL <= 850) {
	        	return 45;
	        } else if (SWL >= 851 && SWL <= 1150) {
	        	return 50;
	        } else if (SWL >= 1151 && SWL <= 1350) {
	        	return 55;
	        } else if (SWL >= 1351 && SWL <= 1650) {
	        	return 60;
	        }else if (SWL >= 1651 && SWL <= 1850) {
	        	return 65;
	        }else if (SWL > 1850) {
	        	return 70;
	        }else{
		...
		//2.a.2
		private double setLoopLongLengthFromRange(double SWL, double surfaceHeight){
			if (SWL <= 499) {
		    	return (surfaceHeight/100 ) * 60 ;
	        } else if (SWL >= 500 && SWL <= 600) {
	        	return (surfaceHeight/100 ) * 60;
	        } else if (SWL >= 601 && SWL <= 850) {
	        	return (surfaceHeight/100 ) * 65;
	        } else if (SWL >= 851 && SWL <= 1150) {
	        	return (surfaceHeight/100 ) * 70;
	        } else if (SWL >= 1151 && SWL <= 1350) {
	        	return (surfaceHeight/100 ) * 75;
	        } else if (SWL >= 1351 && SWL <= 1650) {
	        	return (surfaceHeight/100 ) * 80;
	        }else if (SWL >= 1651 && SWL <= 1850) {
	        	return (surfaceHeight/100 ) * 100;
	        }else if (SWL > 1850) {
	        	return (surfaceHeight/100 ) * 100;
	        }else{
	        	return 0;
		...
		//2.a.3
		private double setLoopShortLengthSFFiveFromRange(double SWL){
			if (SWL <= 499) {
	        	return 25 ;
	        } else if (SWL >= 500 && SWL <= 600) {
	        	return 35;
		...
		//2.a.4
		private double setLoopLongLengthSfFiveFromRange(double SWL, double surfaceHeight){
			if (SWL <= 499) {
		    	return (surfaceHeight/100 ) * 55 ;
	        } else if (SWL >= 500 && SWL <= 600) {
	        	return (surfaceHeight/100 ) * 55;
	        } else if (SWL >= 601 && SWL <= 850) {
	        	return (surfaceHeight/100 ) * 60;
	...
	//2.b 
		...
		//2.b.2
		private double setCrossLoopGramSFFiveFromRange(double bagSWL) {
			// TODO Auto-generated method stub
			if (bagSWL <= 499) {
	         	return 35 ;
	         } else if (bagSWL >= 500 && bagSWL <= 749) {
		...
		// 3.e. Platted Top
			...
			private Double calculateAttachmentOutletBottomSpout(Double bottomSpoutDiameter, Double bottomSpoutGSM,Double bottomSpoutLength, Double surfaceLength, Double surfaceWidth, Double surfaceHeight, String bottomTypeAbbr) {
				...
				private Double calculateThreadForBag(Double surfaceLength, Double surfaceWidth, Double surfaceHeight,
					...
					thread = (((surfaceLength * 2) + (surfaceWidth * 4) + (surfaceHeight * 4) + topThread + bottomThread) / 100) * 5;
					return thread;
				}
				...
				private double calculateSteevdoorStrap(Double surfaceLength, Double surfaceWidth, Double swl, int sfTypeValue) {
					...
					if (sfTypeValue > 5) {
						...
						gramage = setCrossLoopGramSFFiveFromRange(swl);
					}
		...
		//External calculations of bag body and side
		private Double calculateExternalUPanelBody(Double surfaceLength, Double surfaceHeight, Double fabricGSM) {
			Double bagBody = 0.0;
			surfaceLength = surfaceLength + 8;
			double bagCutLength = calculateInternalCutLength(surfaceHeight, surfaceLength);
			bagBody = (surfaceLength * bagCutLength * fabricGSM) / 10000;
			System.out.println("bag body: "+bagBody);
			return bagBody;
		...
		private Double calculateExternalBagSide(Double surfaceLength, Double surfaceWidth, Double surfaceHeight, Double fabricGSM) {
			double bagSide = ((surfaceLength + 8) * (surfaceHeight + 8) * fabricGSM * 2) / 10000;
			System.out.println("bag side = " + bagSide);
			return  bagSide;
		}
}
