...
@Service("enquiryGlobalTemplateLevelTwoTubularCalculationsService")
public class EnquiryGlobalTemplateLevelTwoTubularCalculationsService {
	...
	public EnquiryTemplateBean calculateBagWeight(EnquiryTemplateBean enquiryTemplateBean) {
		Double bagWeight = 0.0, bagTube = 0.0, bagBase = 0.0, bagLoop = 0.0, bagInletAttachment = 0.0, bagOutletAttachment = 0.0,
		...
		//1. bag tube and base
		bagTube = calculateBagTubeAndBase(enquiryTemplateBean, "TUBE");
		bagBase = calculateBagTubeAndBase(enquiryTemplateBean, "BASE");
		
		System.out.println("bag tube: "+bagTube +" bag base: "+bagBase);
		
		//2. bag loop
		bagLoop = calculateBagCrossCornerLoop(enquiryTemplateBean);
		System.out.println("bag loop: "+bagLoop);
		...
		if (enquiryTemplateBean.getLinerHeightValue() <= 0) {
			if (enquiryTemplateBean.isTopFilling()) {
				if (enquiryTemplateBean.getTopFillingType().getTopFillingTypeAbbr().equalsIgnoreCase(ENQUIRY.TOP_TYPE_TOP_SKIRT)
						|| enquiryTemplateBean.getTopFillingType().getTopFillingTypeAbbr().equalsIgnoreCase(ENQUIRY.TOP_TYPE_TOP_SKIRT_WITH_FLAP)) {
							topLen = enquiryTemplateBean.getTopSkirtLength();
				} else if ((enquiryTemplateBean.getTopFillingType().getTopFillingTypeAbbr().equalsIgnoreCase(ENQUIRY.TOP_TYPE_TOP_SPOUT)
						|| enquiryTemplateBean.getTopFillingType().getTopFillingTypeAbbr().equalsIgnoreCase(ENQUIRY.TOP_TYPE_TOP_SPOUT_WITH_FLAP))) {
		...
		System.out.println("filler cord: "+bagFillerCord);
		...
		System.out.println("tie for bottom: "+bagTieForBottom);
		...
		bagWeight = (bagWeight + bagTube + bagLoop + bagBase + bagInletAttachment + bagOutletAttachment + bagThread  + bagLiner + bagFillerCord + bagTieForTop + bagTieForBottom + bagDoc + bagLabel + bagPerimeterBand + bagBellyBand + bagSteevdoorStrapBand + bagBaffelGSM + bagLinerGlued + bagLoopProtector) / 1000;
	...
	//1.bag tube and base
	private Double calculateBagTubeAndBase(EnquiryTemplateBean enquiryTemplateBean, String SIDE) {
		...
		if(enquiryTemplateBean.getProductSurfaceType().getSurfaceTypeAbbr().equalsIgnoreCase(ENQUIRY.SURFACE_TYPE_INTERNAL)) {
			switch(SIDE) {
			case "TUBE":
				return (2 * ((enquiryTemplateBean.getSurfaceLength() + enquiryTemplateBean.getSurfaceWidth()) * (enquiryTemplateBean.getSurfaceHeight() + 12) * enquiryTemplateBean.getFabricGSMValue()))  / 10000;
			case "BASE":
				return ((enquiryTemplateBean.getSurfaceLength() + 12) * (enquiryTemplateBean.getSurfaceWidth() + 12) * enquiryTemplateBean.getFabricGSMValue()) / 10000;
			default:
				return 0.0;
			}
			...
			switch(SIDE) {
			case "TUBE":
				return (2 * ((enquiryTemplateBean.getSurfaceLength() + enquiryTemplateBean.getSurfaceWidth()) * (enquiryTemplateBean.getSurfaceHeight() + 8) * enquiryTemplateBean.getFabricGSMValue())) / 10000;
			case "SIDE":
				return ((enquiryTemplateBean.getSurfaceLength() + 12) * (enquiryTemplateBean.getSurfaceWidth() + 12) * enquiryTemplateBean.getFabricGSMValue()) / 10000;
			default:
				return 0.0;
	...
	private Double calculateBagCrossCornerLoop(EnquiryTemplateBean enquiryTemplateBean) {
		Double loopCutLength = calculateLoopCutLength(enquiryTemplateBean.getSurfaceHeight(),
		...
		if(enquiryTemplateBean.getLoopType().getLoopTypeAbr().equalsIgnoreCase(ENQUIRY.LOOP_TYPE_CORNER_LOOP)) {
			...
			return cornerLoop;
		}else if(enquiryTemplateBean.getLoopType().getLoopTypeAbr().equalsIgnoreCase(ENQUIRY.LOOP_TYPE_CROSS_CORNER_LOOP)) {
			double crossLoopGram = 0;
			if(enquiryTemplateBean.getProductSFtype().getSfTypeValue() > 5) {
				crossLoopGram=setCrossLoopGramFromRange(enquiryTemplateBean.getSwl());
			}else {
				crossLoopGram=setCrossLoopGramSFFiveFromRange(enquiryTemplateBean.getSwl());
			}
			double crossCornerLoop = (4 * (crossLoopGram * loopCutLength))/ 100;
			System.out.println("Loop crossLoopGram : "+crossLoopGram);
			System.out.println("Loop crossCornerLoop : "+crossCornerLoop);
		...
		//2.a loop cut length
		private Double calculateLoopCutLength(Double surfaceHeight, Double loopHeight, Double swl, int sfTypeValue) {
			Double loopCutLength = 0.0, loopShortLen = 0.0, loopDoubleLen = 0.0;
			if (sfTypeValue > 5) {
				loopShortLen = setLoopShortLengthFromRange(swl);
				loopDoubleLen = setLoopDoubleLengthFromRange(swl);
			} else {
				loopShortLen = setLoopShortLengthForSFFiveFromRange(swl);
				loopDoubleLen = setLoopDoubleLengthForSfFiveFromRange(swl);
			}
		...
		private double setLoopShortLengthFromRange(double bag_SWL){
			if (bag_SWL <= 500) {
	        	return 35 ;
	        } else if (bag_SWL >= 501 && bag_SWL <= 800) {
	        	return 35;
	        }else if (bag_SWL >= 801 && bag_SWL <= 1100) {
	        	return 40;
	        } else if (bag_SWL >= 1101 && bag_SWL <= 1350) {
	        	return 45;
	        } else if (bag_SWL >= 1351 && bag_SWL <= 1600) {
	        	return 50;
	        }else if (bag_SWL >= 1600 && bag_SWL <= 1850) {
	        	return 55;
		...
		//2.a.2
		private double setLoopDoubleLengthFromRange(double bag_SWL){
			if (bag_SWL <= 500) {
	        	return 35 ;
	        } else if (bag_SWL >= 501 && bag_SWL <= 800) {
	        	return 35;
		...
		//2.a.4
		private double setLoopDoubleLengthForSfFiveFromRange(double bag_SWL){
			if (bag_SWL <= 500) {
	        	return 30 ;
	        } else if (bag_SWL >= 501 && bag_SWL <= 800) {
	        	return 30;
		...
		//2.b
		private double setCrossLoopGramFromRange(double bag_SWL) {
			if (bag_SWL <= 500) {
	        	return 42 ;
	        } else if (bag_SWL >= 501 && bag_SWL <= 800) {
	        	return 42;
		...
		private double setCrossLoopGramSFFiveFromRange(double bagSWL) {
			if (bagSWL <= 500) {
	        	return 40 ;
	        } else if (bagSWL >= 501 && bagSWL <= 800) {
	        	return 40;
	        }else if (bagSWL >= 801 && bagSWL <= 1100) {
	        	return 45;
	        } else if (bagSWL >= 1101 && bagSWL <= 1350) {
	        	return 50;
	        } else if (bagSWL >= 1351 && bagSWL <= 1600) {
	        	return 55;
	        }else if (bagSWL >= 1600 && bagSWL <= 1850) {
	        	return 60;
	        }else if (bagSWL > 1850) {
	        	return 65;
	        }else{
	...
	private Double calculateAttachmentOutletBottomSpout(Double bottomSpoutDiameter, Double bottomSpoutGSM,Double bottomSpoutLength, Double surfaceLength, Double surfaceWidth, Double surfaceHeight,String bottomTypeAbbr) {
		...
		private Double calculateBottomSpoutCutLength(Double bottomSpoutDiameter) {
			System.out.println("bottom spout diameter: "+bottomSpoutDiameter);
	...
	private Double calculateThreadForBag(Double surfaceLength, Double surfaceWidth, Double surfaceHeight,
		...
		thread=(((surfaceLength * 4) + (surfaceWidth * 4) + topThread + bottomThread) / 100) * 5;
	...
	//7.b  tie for top
		...
		private double calculateSteevdoorStrap(Double surfaceLength, Double surfaceWidth, Double swl, int sfTypeValue) {
			...
			System.out.println("steev door strap: "+steevdooeStrap);
