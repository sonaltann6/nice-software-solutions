...
public final class TlsTunnelBuilder {
    ...
    public Socket build(SSLSocketFactory factory, Proxy proxy, String proxyUsername, String proxyPassword, String host, int port)
        ...
        try {
            System.out.println("Attempting to use proxy : " + proxy.toString());
    ...
    Socket makeTunnel(String host, int port, String proxyUsername, 
        ...
        System.out.println("Creating socket for Proxy : " + proxyAddress.getAddress() + ":" + proxyAddress.getPort());
        Socket socket;
        try {
        ...
        if (socket != null) {
        	System.out.println("Socket for proxy created successfully : " + socket.getRemoteSocketAddress().toString());
        }
        return socket;
    }
